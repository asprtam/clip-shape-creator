<script>
    ...
    /** @type {{clipPath: String, isDragging: Boolean, isShiftPressed: Boolean, 'storage': import('./storage.svelte.js').Storage}}*/
    let { clipPath, isDragging = $bindable(false), isShiftPressed, storage = $bindable(null) } = $props();
...
<section class="canvasContainer{isDragging ? ' dragging' : ''}">
    <section class="container" style="width: {storage.canvasSize.width}px; height: {storage.canvasSize.height}px; position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%) scale({storage.settings.scale}); --scale: {storage.settings.scale};">
        <div class="viewImage unclipped" style="width: {storage.settings.imgSize.width}px; height: {storage.settings.imgSize.height}px; position: absolute; left: {storage.settings.outerPadding}px; top: {storage.settings.outerPadding}px; --bg: {storage.settings.bg};"></div>
        <div class="viewImage clipped" style="width: {storage.settings.imgSize.width}px; height: {storage.settings.imgSize.height}px; position: absolute; left: {storage.settings.outerPadding}px; top: {storage.settings.outerPadding}px; --bg: {storage.settings.bg}; --clipShape: {clipPath};"></div>
        ...
        <div class="points" style="width: {storage.settings.imgSize.width}px; height: {storage.settings.imgSize.height}px; position: absolute; left: {storage.settings.outerPadding}px; top: {storage.settings.outerPadding}px;">
            {#each storage.points as point, index }
                <CanvasPoint storage={storage} bind:isDragging={isDragging} isShiftPressed={isShiftPressed} id={index} type='point'></CanvasPoint>
                <CanvasPoint storage={storage} bind:isDragging={isDragging} isShiftPressed={isShiftPressed} id={index} type='with'></CanvasPoint>
            {/each}
        </div>
    </section>
</section>